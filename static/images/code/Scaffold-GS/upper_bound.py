...
def fine_tune(scene, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, mask, first_iter, wandb=None, logger=None):
    ...
    progress_bar = tqdm(range(first_iter, first_iter + opt.fine_tune_iters), desc="Training progress")
    first_iter += 1
...
def prune_fine_tune(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None):
    ...
    gaussians.load_mlp_checkpoints(f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/Scaffold-GS/outputs/{args.dataset_name}/{args.scene}/baseline/point_cloud/iteration_30000/')
    ...
    offset_mask = None
    anchor_mask = None
    offset_masks, anchor_masks = [], []
    k = 0
    #num_offsets = gaussians._anchor.shape[0] * gaussians.n_offsets
    ...
    scene = fine_tune(scene, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, None, first_iter, wandb=None, logger=logger)
    ...
    for i in range(args.num_levels): 
        ...
        k = k_s[i]
        anchor_mask, offset_mask = scene.gaussians.gradient_prune(k)
        ...
        scene = fine_tune(scene, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, anchor_mask, first_iter, wandb=None, logger=logger)
...
if __name__ == "__main__":
    ...
    # dataset_name = ['tandt', 'nerf_real_360', 'db']
    ...
    #     for scene in scenes[dataset_name]:
            ...
            #args.model_path = os.path.join(os.getcwd(), 'prova', scene)
            # args.start_checkpoint = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/Scaffold-GS/outputs/{args.dataset_name}/{args.scene}/baseline/chkpnt30000.pth'
    args.eval = True
    # enable logging
    #args.model_path = 'prova'
    ...
    ply_path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/Scaffold-GS/outputs/{args.dataset_name}/{args.scene}/baseline/point_cloud/iteration_30000/point_cloud.ply'

    args.voxel_size=0.001 
    if args.dataset_name == 'db':
        args.voxel_size=0.005
    elif args.dataset_name == 'tandt':
        args.voxel_size = 0.1
    args.update_init_factor=16
    args.appearance_dim=0
    args.ratio=1
