...
def write(string):
    with open(f'{args.model_path}/results.csv', 'a') as f:
        print(string, file=f)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                save_path = scene.save(iteration)
            ...
            if iteration == testing_iterations[-1]:
                ...
                test(None, iteration, None, None, l1_loss, None, testing_iterations, scene, render, (pipe, background), 
                        dump_images=dump_images, eval_lpips=eval_lpips, mask=None, level=1, size=size)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    #             # if config['name'] == 'test':
    #             #     write(f'{psnr_test}, {scene.gaussians._xyz.shape[0]}')
...
if __name__ == "__main__":
    ...
    if 'drjohnson' in args.source_path:
        args.opacity_reg = 0.001
