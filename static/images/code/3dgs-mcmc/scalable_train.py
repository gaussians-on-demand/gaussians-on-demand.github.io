...
def iterative_masking(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, fine_tune=False):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians)
    
    gaussians.load_ply(os.path.join(args.pretrained_dir, 'point_cloud', 'iteration_30000', 'point_cloud.ply'))
    ...
    for i in range(args.num_levels):
        ...
        if i == 0:
            with torch.no_grad():       
                ...
                scene.gaussians._degrees = scene.gaussians._degrees[mask].clone().detach()
...
if __name__ == "__main__":
    ...
    if args.test_only:
        masks = torch.load(f'{args.model_path}/masks.pt')
        args.test_iterations = [1]
        args.save_iterations = [-1]
        args.checkpoint_iterations = [-1]
        args.iterations = 1
        
        args.model_path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/3dgs-mcmc/multi_level_models/reduced/{args.scene}'
        train(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, masks)
    

    scene, masks = iterative_masking(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.fine_tune)
