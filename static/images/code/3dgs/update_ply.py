...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", "-p", help="Path of the ply file to convert", type=str)
    parser.add_argument("--name", "-n", help="Name of the newly created point cloud", type=str)
    ...
    scenes = ['truck','drjohnson', 'playroom', 'train']
    
    for scene in scenes:

        path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/multi_level_models/{scene}/G=5_L=8_min=100000_max=0.75_ws=False/point_cloud/iteration_60000'
        plydata = PlyData.read(f'{path}/point_cloud.ply')
        if validate_new_format(plydata):
            ...
            name = f'{path}/point_cloud_new.ply'
            convert_ply(plydata, Path(path).parent / Path(name), max_sh_order=args.max_sh_order)
            print("Done")
        
# python3 update_old_ply_format.py -p /scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/multi_level_models/bonsai/G=5_L=8_min=0.05_max=0.75_ws=False/point_cloud/iteration_60000/point_cloud.ply -n \
# /scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/multi_level_models/bonsai/G=5_L=8_min=0.05_max=0.75_ws=False/point_cloud/iteration_60000/point_cloud_up.ply
