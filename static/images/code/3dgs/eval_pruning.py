...
def build_masks(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, fine_tune=False):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    
    write(f'### PRUNE TYPE={args.prune_type}')
    ...
    for i in range(args.num_levels):
        ...
        k = k_s[i] 
        mask = gaussians.gradient_prune(k, prune_type=args.prune_type)
        keep = args.iterative
        masks.append(mask)
        ...
        # if fine_tune:
        #     fine_tuning(scene, opt, gaussians, pipe, background, mask, progress_bar, first_iter)
...
def fine_tune(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, scene, masks, load_iter=None):
    first_iter = 0 if args.reduced else 30_000
    tb_writer = prepare_output_and_logger(dataset)
    ...
    if args.reduced:
        ...
        with torch.no_grad():
            ...
            for i, mask in enumerate(masks):
                gaussians_cpy = copy.deepcopy(gaussians)
                gaussians_cpy._xyz = gaussians._xyz[mask]
                gaussians_cpy._opacity = gaussians._opacity[mask]
                gaussians_cpy._rotation = gaussians._rotation[mask]
                gaussians_cpy._scaling = gaussians._scaling[mask]
                gaussians_cpy._features_dc = gaussians._features_dc[mask]
                gaussians_cpy._features_rest = gaussians._features_rest[mask]
                
                save_path = os.path.join(args.model_path, f'data_{i}.npz')
                save_paths.append(save_path)
                gaussians_cpy.save_npz(save_path)
            ...
            for path in save_paths:
                ...
                eval_lpips = False
                dump_images = False
                test(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
                        testing_iterations, scene, render, (pipe, background), wandb, None, None, eval_lpips=eval_lpips,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prune_type", type=str, default=None) #random, grad_position, opacity_based, grad_(w/o feat), grad_all, one_shot, iterative
    ...
    for dataset_name in scenes:
        for scene in scenes[dataset_name]:
            for i, prune_type in enumerate(prune_types):
                args.iterative = False
                # if i == len(prune_types) - 1:
                #     args.iterative = False
                args.source_path = f"/scratch/nerf/dataset/{dataset_name}/{scene}"
                args.prune_type = prune_type
                args.model_path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/multi_level_models/{scene}/pruning'
                ...
                ply_path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/output/{scene}/point_cloud/iteration_30000/point_cloud_new.ply'
                #ply_path = f'/scratch/gaussian-splatting/gaussian-splatting/LightGaussian/gaussian-splatting/output/{args.scene}/sparsify=True/point_cloud/iteration_30000/point_cloud.ply'
                
    
                #assert args.start_checkpoint != None, 'You need to provide a checkpoint'
                build_masks(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.fine_tune)
